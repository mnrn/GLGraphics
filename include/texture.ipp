    ...
    for (std::size_t face = 0; face < texture.faces(); face++) {
      for (std::size_t level = 0; level < texture.levels(); level++) {
        ...
        target = gli::is_target_cube(texture.target())
                     ? static_cast<GLenum>(GL_TEXTURE_CUB_MAP_POSITIVE_X + face)
                     : target;
        ...
        case gli::TARGET_1D_ARRAY:
          gli::is_compressed(texture.format())
              ? glCompressedTexSubImage2D(
                    target, static_cast<GLint>(level), 0, 0, extent.x, layerGL,
                    format.Internal, static_cast<GLsizei>(texture.size(level)),
                    texture.data(layer, face, level))
              : glTexSubImage2D(target, static_cast<GLint>(level), 0, 0,
                                extent.x, layerGL, format.External, format.Type,
                                texture.data(layer, face, level));
          break;
        case gli::TARGET_2D:
          [[fallthrough]];
        case gli::TARGET_CUBE:
          gli::is_compressed(texture.format())
              ? glCompressedTexSubImage2D(
                    ...
                    texture.data(layer, face, level))
              : glTexSubImage2D(target, static_cast<GLint>(level), 0, 0,
                                extent.x, extent.y, format.External,
                                format.Type, texture.data(layer, face, level));
          break;
        case gli::TARGET_3D:
          gli::is_compressed(texture.format())
              ? glCompressedTexSubImage3D(
                    target, static_cast<GLint>(level), 0, 0, 0, extent.x,
                    extent.y, extent.z, format.Internal,
                    static_cast<GLsizei>(texture.size(level)),
                    texture.data(layer, face, level))
              : glTexSubImage3D(target, static_cast<GLint>(level), 0, 0, 0,
                                extent.x, extent.y, extent.z, format.External,
                                format.Type, texture.data(layer, face, level));
          break;
        case gli::TARGET_2D_ARRAY:
          [[fallthrough]];
        case gli::TARGET_CUBE_ARRAY:
          gli::is_compressed(texture.format())
              ? glCompressedTexSubImage3D(
                    target, static_cast<GLint>(level), 0, 0, 0, extent.x,
                    extent.y, layerGL, format.Internal,
                    static_cast<GLsizei>(texture.size(level)),
                    texture.data(layer, face, level))
              : glTexSubImage3D(target, static_cast<GLint>(level), 0, 0, 0,
                                extent.x, extent.y, layerGL, format.External,
                                format.Type, texture.data(layer, face, level));
          break;
        default:
          BOOST_ASSERT_MSG(false, "invalid texture target!");
