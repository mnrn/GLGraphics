...
uniform sampler2DShadow ShadowMaps[kCascadesNum];
uniform float SplitPlanes[kCascadesNum];
...
void ShadeWithShadow() {
    ...
    float dist = 0.0;
    for (int i = 0; i < 1; i++) {
        if (ShadowCoords[i].z >= 0.0){
            shadow = textureProj(ShadowMaps[i], ShadowCoords[i]);
        }
    }

    // ピクセルが影の中にある場合、Ambient Light (環境光)のみ使用することになります。
    vec4 color = vec4(diffSpec * shadow + amb, 1.0);
    //color = vec4(diffSpec + amb, 1.0);
